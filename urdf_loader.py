...
from sapien import ActorBuilder, ArticulationBuilder
from sapien.wrapper.articulation_builder import MimicJointRecord
...
from sapien.wrapper.urchin import URDF, Link
...
class URDFLoader:
    def __init__(self, scene: sapien.Scene, fix_root_link: bool = True, load_collision=True):
        self.fix_root_link = fix_root_link
        self.scene = scene

        self.load_collision = load_collision
    ...
    def _build_link(self, link: Link, link_builder: Union[ActorBuilder, ArticulationBuilder]):
        # inertial
        if (
            link.inertial
        ...
        for visual in link.visuals:
            ...
            if visual.geometry.mesh:
                ...
                # print()
                f_tmp = _try_very_hard_to_find_file(
                        visual.geometry.mesh.filename,
                        self.urdf_dir,
                        self.package_dir,
                    )
                import trimesh as tm
                mesh = tm.load(f_tmp, force='mesh', process=False)
                mesh.apply_scale(scale)
                # print(scale)
                mesh.export(f_tmp[:-4] + '.glb')

        # collision shapes
        if not self.load_collision:
            return
    ...
    def load(
        ...
        articulations = []
        for b in articulation_builders:
            articulations.append(b.build(build_mimic_joints=False))
